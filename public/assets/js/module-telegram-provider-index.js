...
var ModuleTelegramProvider = {
  ...
  checkStatuses: function checkStatuses() {
    $.get('/pbxcore/api/modules/' + className + '/statuses', function (response) {
      ...
      for (var id in response.data) {
        var uriButton = $('#ModuleTelegramProvider-table tr[id=' + id + '] button.ui.button');
        var loginButton = $('#ModuleTelegramProvider-table tr[id=' + id + '] a[data-name="login-button"] i');
        var elIcon = $('#ModuleTelegramProvider-table tr[id=' + id + '] i.phone.icon');
   ...
   * @param id
   */
  startAuth: function startAuth(id) {
    $("#error-message").hide();
      ...
      createdRow: function createdRow(row, data) {
        ...
        for (var key in data) {
          ...
          if (key === 'rowIcon') {
            ...
            var templateDeleteButton = '<div class="ui small basic icon buttons action-buttons">' + '<button data-name="uri-button" class="ui button clipboard disability" data-tooltip="' + globalTranslate.module_telegram_providerCopy + '"  data-position="left center" data-clipboard-text="' + uri + '" style="display: none;">sip:' + uri + '</button>' + '<a data-name="login-button"  href="" class="ui button popuped"><i class="icon sign-in"></i></a>' + '<a data-name="delete-button" href="' + window[className].deleteRecordAJAXUrl + '/' + data.DT_RowId + '" data-value = "' + data.DT_RowId + '"' + ' class="ui button delete two-steps-delete popuped" data-content="' + globalTranslate.bt_ToolTipDelete + '">' + '<i class="icon trash red"></i></a></div>';
